...
namespace framework
{
    namespace serializable
    {
        ...
        {
            template <typename T>
            struct big_endian_impl <T, typename std::enable_if <std::is_integral <T>::value, void>::type>
            {
...
#else
                    static_assert(!std::is_same <T, T>::value, "big_endian does not support host endianness");
#endif
                }
            ...
            template <typename T>
            struct big_endian_impl <T, typename std::enable_if <std::is_floating_point <T>::value, void>::type>
            {
                static void to_host (T& value)
                {
                    (void)value; // suppress unused value warning
...
#else
                    static_assert(false, "big_endian does not support host endianness");
#endif
                }
